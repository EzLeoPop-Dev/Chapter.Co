  ...
  return (
    <div className="min-h-screen flex bg-[#F2EEE7] text-[#1A1A1A] selection:bg-[#C8861A] selection:text-white font-[-apple-system,BlinkMacSystemFont,'Inter','Segoe_UI',Roboto,sans-serif]">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-4 md:p-8 relative overflow-hidden">
        ...
        <div className="absolute bottom-[20%] right-[20%] w-96 h-96 bg-primary rounded-full mix-blend-multiply filter blur-[128px] opacity-30 animate-blob animation-delay-2000"></div>
        ...
        <div className="w-full max-w-md backdrop-blur-xl bg-white/40 border border-white/60 rounded-[2.5rem] p-10 shadow-[0_8px_30px_rgb(0,0,0,0.06)] relative z-10 text-center">
          ...
          <Link href="/" className="inline-block text-[24px] font-normal tracking-[-0.48px] bg-clip-text text-transparent bg-gradient-to-r from-[#1A1A1A] to-[#1A1A1A] mb-8 hover:opacity-80 transition-opacity">
            Chapter.Co
          </Link>
          
          <div className="mx-auto flex items-center justify-center h-20 w-20 rounded-full bg-primary/80 border border-primary/50 mb-6 shadow-sm">
            <svg className="h-10 w-10 text-[#C8861A]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
            </svg>
          </div>
          
          <h1 className="text-[32px] font-normal tracking-[-0.64px] text-[#1A1A1A] mb-2">Signed Out</h1>
          <p className="text-[#1A1A1A] mb-8">You have successfully logged out of your account.</p>

          <Link 
            href="/auth/login"
            className="inline-block w-full py-3.5 px-4 rounded-2xl bg-white/60 backdrop-blur-sm border border-white text-[#1A1A1A] font-medium hover:bg-white hover:shadow-md hover:-translate-y-0.5 transition-all duration-300"
          >
            Return to Login
          </Link>
        </div>
