  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-3xl font-black text-[#26251e]">ตรวจสอบการชำระเงิน (Payment Verification)</h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {payments.map((payment, idx) => (
          <div key={idx} className="bg-white/70 backdrop-blur-xl rounded-[2.5rem] p-6 shadow-sm border border-white/80 flex flex-col md:flex-row gap-6">
            ...
            {/* Slip Image Mockup */}
            <div className="w-full md:w-1/3 aspect-[3/4] bg-white/50 rounded-2xl border-2 border-dashed border-[#e6e5e0] flex items-center justify-center text-[#a09c92] group cursor-pointer hover:bg-white hover:border-orange-300 transition-all relative overflow-hidden">
               <span className="font-bold text-sm group-hover:text-orange-500">ดูสลิปโอนเงิน</span>
            ...
            <div className="flex-1 flex flex-col">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="font-bold text-xl text-[#26251e]">{payment.orderId}</h3>
                  <p className="text-sm font-medium text-[#5a5852] mt-1">{payment.date}</p>
                </div>
                <span className="px-3 py-1.5 bg-orange-100 text-orange-600 text-xs font-bold rounded-full border border-orange-200">
                  {payment.status}
                </span>
              </div>
              ...
              <div className="space-y-4 flex-1 mt-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold text-[#26251e]">{payment.customer}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[#a09c92] font-bold">ช่องทาง:</span>
                  <span className="font-bold text-[#26251e]">{payment.method} ({payment.bank})</span>
                </div>
                <div className="flex justify-between items-center text-sm bg-orange-50 p-4 rounded-2xl border border-orange-100 mt-4">
                  <span className="text-orange-700 font-bold">ยอดโอน (อ้างอิงจากระบบ):</span>
                  <span className="font-black text-orange-600 text-xl">${payment.amount.toFixed(2)}</span>
                </div>
              </div>
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button className="flex-1 bg-white border border-[#e6e5e0] text-[#5a5852] hover:bg-red-50 hover:text-red-500 hover:border-red-200 font-bold py-3 rounded-xl transition-colors text-sm shadow-sm">
                  ปฏิเสธ
                </button>
              </div>
            </div>
      ...
      {payments.length === 0 && (
         <div className="bg-white/70 backdrop-blur-xl rounded-[2.5rem] p-16 text-center border border-white/80 shadow-sm flex flex-col items-center">
           ...
           <h3 className="text-2xl font-bold text-[#26251e] mb-2">ไม่มีรายการรอตรวจสอบ</h3>
           <p className="text-[#5a5852] font-medium">คุณตรวจสอบการชำระเงินทั้งหมดเรียบร้อยแล้ว</p>
         </div>
      )}
    </div>
  );
