  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h2 className="text-3xl font-black text-[#26251e]">จัดการสต็อกสินค้า (Stock Management)</h2>
      ...
      <div className="bg-white/70 backdrop-blur-xl rounded-[2.5rem] shadow-sm border border-white/80 overflow-hidden">
        <div className="p-5 border-b border-[#e6e5e0] flex flex-col md:flex-row gap-4 bg-white/40">
          <input type="text" placeholder="ค้นหาชื่อหนังสือ, รหัสสินค้า..." className="px-5 py-3 border border-[#e6e5e0] rounded-xl text-sm flex-1 focus:outline-none focus:ring-2 focus:ring-orange-300 bg-white/60 font-medium" />
          <select className="px-5 py-3 border border-[#e6e5e0] rounded-xl text-sm bg-white/60 focus:outline-none focus:ring-2 focus:ring-orange-300 font-bold text-[#5a5852]">
            <option>ทุกหมวดหมู่</option>
            <option>จิตวิทยาและการพัฒนาตัวเอง</option>
            <option>ประวัติศาสตร์</option>
            <option>การ์ตูน (Manga)</option>
          </select>
          <select className="px-5 py-3 border border-orange-200 rounded-xl text-sm bg-orange-50 focus:outline-none focus:ring-2 focus:ring-orange-300 text-orange-600 font-bold">
            ...
            <tbody className="divide-y divide-[#e6e5e0]/50">
              {stockItems.map((item, idx) => (
                <tr key={idx} className="hover:bg-white/80 transition-colors">
                  ...
                  <td className="p-5 font-bold text-[#26251e]">{item.title}</td>
                  <td className="p-5 text-sm font-medium text-[#5a5852]">{item.category}</td>
                  <td className="p-5 text-sm font-black text-[#f54e00] text-right">${item.price.toFixed(2)}</td>
                  <td className="p-5 text-center font-black text-xl">
                    <span className={item.stock === 0 ? 'text-red-500' : item.stock <= 5 ? 'text-orange-500' : 'text-[#26251e]'}>
                      {item.stock}
                    </span>
                  </td>
                  <td className="p-5 text-center">
                    <span className={`px-4 py-1.5 text-[11px] font-bold rounded-full uppercase tracking-wider border ${
                      ...
                      item.status === 'ใกล้หมด' ? 'bg-orange-50 text-orange-600 border-orange-200' : 
                  ...
                  <td className="p-5 flex items-center justify-center space-x-3">
                    <button className="p-2.5 bg-white border border-[#e6e5e0] hover:bg-orange-50 text-[#5a5852] hover:text-orange-600 hover:border-orange-200 rounded-xl transition-all shadow-sm" title="ปรับสต็อก">
                    ...
                    <button className="p-2.5 bg-white border border-[#e6e5e0] hover:bg-[#f7f7f4] text-[#5a5852] hover:text-[#26251e] rounded-xl transition-all shadow-sm" title="แก้ไขข้อมูล">
