    ...
    <div className="min-h-screen bg-[#f7f7f4] text-[#26251e] font-[-apple-system,BlinkMacSystemFont,'Inter','Segoe_UI',Roboto,sans-serif] relative selection:bg-[#f54e00] selection:text-white p-4 md:p-8 flex flex-col">
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
        ...
        <div className="mt-8 mb-8 flex items-center justify-between">
          <h1 className="text-3xl font-black text-[#26251e]">การชำระเงิน</h1>
          <div className="hidden md:flex items-center space-x-4 text-sm font-bold text-[#a09c92]">
            ...
            <span className="flex items-center text-orange-500"><div className="w-6 h-6 rounded-full bg-orange-500 text-white flex items-center justify-center mr-2">2</div> ชำระเงิน</span>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="flex-[2] space-y-6">
            ...
            <div className="bg-white/70 backdrop-blur-xl border border-white/80 rounded-[2.5rem] p-8 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                <button 
                  ...
                  className={`p-5 rounded-2xl border-2 text-left transition-all ${paymentMethod === 'qr' ? 'border-orange-500 bg-orange-50' : 'border-[#e6e5e0] bg-white hover:border-orange-300'}`}
                >
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="font-bold">Thai QR PromptPay</h3>
                    {paymentMethod === 'qr' && <div className="w-4 h-4 rounded-full bg-orange-500"></div>}
                  </div>
                  <p className="text-sm text-[#a09c92] font-medium">สแกนจ่ายผ่านแอปธนาคาร</p>
                </button>
                <button 
                  onClick={() => setPaymentMethod('card')}
                  className={`p-5 rounded-2xl border-2 text-left transition-all ${paymentMethod === 'card' ? 'border-orange-500 bg-orange-50' : 'border-[#e6e5e0] bg-white hover:border-orange-300'}`}
                >
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="font-bold">Credit / Debit Card</h3>
                    {paymentMethod === 'card' && <div className="w-4 h-4 rounded-full bg-orange-500"></div>}
                  </div>
              ...
              {paymentMethod === 'qr' && (
                <div className="bg-white rounded-2xl p-8 border border-[#e6e5e0] flex flex-col items-center justify-center animate-fadeIn text-center">
                  ...
                  <p className="text-orange-500 font-bold text-xl mt-4">ยอดโอน: $37.99</p>
                </div>
              )}
              ...
              {paymentMethod === 'card' && (
                <div className="bg-white rounded-2xl p-8 border border-[#e6e5e0] animate-fadeIn space-y-4">
                  <div>
                    ...
                    <input type="text" className="w-full bg-[#f7f7f4] border border-[#e6e5e0] rounded-xl px-4 py-3 font-medium focus:outline-none focus:border-orange-300" placeholder="0000 0000 0000 0000" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-bold text-[#a09c92] uppercase tracking-wider mb-2">วันหมดอายุ</label>
                      <input type="text" className="w-full bg-[#f7f7f4] border border-[#e6e5e0] rounded-xl px-4 py-3 font-medium focus:outline-none focus:border-orange-300" placeholder="MM/YY" />
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-[#a09c92] uppercase tracking-wider mb-2">CVV</label>
                      <input type="text" className="w-full bg-[#f7f7f4] border border-[#e6e5e0] rounded-xl px-4 py-3 font-medium focus:outline-none focus:border-orange-300" placeholder="123" />
                    </div>
                  </div>
                </div>
              )}
            ...
            {paymentMethod === 'qr' && (
              <div className="bg-white/70 backdrop-blur-xl border border-white/80 rounded-[2.5rem] p-8 shadow-sm">
                ...
                <div className="border-2 border-dashed border-[#d0cdc5] rounded-2xl p-8 text-center cursor-pointer hover:border-orange-500 hover:bg-orange-50 transition-colors">
          ...
          <div className="flex-1">
            <div className="bg-white/80 backdrop-blur-xl border border-white rounded-[2.5rem] p-8 shadow-xl sticky top-8">
              ...
              <Link href="/orders" className="w-full flex items-center justify-center bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-bold py-4 rounded-2xl shadow-lg hover:shadow-orange-500/40 transition-all hover:-translate-y-1">
                ยืนยันการชำระเงิน
              </Link>
            </div>
          </div>
