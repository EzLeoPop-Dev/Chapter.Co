    ...
    <>
      <header className="mb-6 flex-shrink-0 w-full relative z-40">
        <div className="flex justify-between items-center mb-6 px-4">
          <Link href="/" className="text-[36px] font-normal tracking-[-0.72px] bg-clip-text text-transparent bg-gradient-to-r from-[#26251e] to-[#5a5852] hover:opacity-80 transition-opacity">
            Chapter.Co
          </Link>
          <div className="flex items-center space-x-3">
            <button onClick={() => setIsSideMenuOpen(true)} className="group flex items-center p-2.5 text-[#5a5852] hover:text-[#26251e] hover:bg-white/60 rounded-full backdrop-blur-sm transition-all">
              <span className="w-0 overflow-hidden opacity-0 group-hover:w-[60px] group-hover:opacity-100 group-hover:mr-2 transition-all duration-300 text-[14px] font-medium whitespace-nowrap text-right text-orange-500">
                หมวดหมู่
              </span>
            ...
            <div className="flex items-center relative">
              <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isSearchOpen ? 'w-48 opacity-100 mr-2' : 'w-0 opacity-0'}`}>
                <input
                  ...
                  className="w-full bg-white/50 border border-[#e6e5e0] backdrop-blur-sm rounded-full py-1.5 px-4 text-sm text-[#26251e] focus:outline-none focus:ring-2 focus:ring-orange-300 transition-all"
                />
              </div>
              <button
                onClick={() => setIsSearchOpen(!isSearchOpen)}
                className={`p-2.5 rounded-full backdrop-blur-sm transition-all z-10 ${isSearchOpen ? 'bg-orange-100 text-[#f54e00]' : 'text-[#5a5852] hover:text-[#26251e] hover:bg-white/60'}`}
              >
        ...
        <div className="backdrop-blur-xl bg-[#ffffff]/50 border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-full px-8 py-4 flex flex-wrap justify-between items-center transition-all">
          <nav className="flex space-x-8 text-[14px] font-medium text-[#5a5852]">
            <Link href="/" className="hover:text-[#26251e] hover:border-b-2 hover:border-orange-200 pb-1 transition-all">หน้าหลัก</Link>
            <a href="#" className="hover:text-[#26251e] hover:border-b-2 hover:border-orange-200 pb-1 transition-all">หนังสือแนะนำ</a>
            <Link href="/shop" className="hover:text-[#26251e] hover:border-b-2 hover:border-orange-200 pb-1 transition-all">เลือกซื้อหนังสือ</Link>
          </nav>
          <div className="flex items-center space-x-5 mt-4 sm:mt-0">
            <Link href="/coupons" className="flex items-center space-x-1.5 bg-gradient-to-r from-orange-400 to-amber-500 text-white px-4 py-1.5 rounded-full hover:shadow-lg hover:shadow-orange-500/30 hover:-translate-y-0.5 transition-all font-medium text-[13px]">
            ...
            <Link href="/profile" className="text-[#5a5852] hover:text-orange-500 transition-colors">
            ...
            <Link href="/cart" className="text-[#5a5852] hover:text-orange-500 transition-colors relative p-2.5 rounded-full hover:bg-white/60">
              <span className="absolute top-1 right-1 bg-[#f54e00] text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">2</span>
      ...
      <div className={`fixed inset-0 z-50 flex transition-all duration-300 ${isSideMenuOpen ? 'visible' : 'invisible'}`}>
        ...
        <div className={`relative w-80 max-w-sm bg-[#f7f7f4]/95 backdrop-blur-xl h-full shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${isSideMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="p-6 border-b border-[#e6e5e0] flex justify-between items-center h-[76px]">
            {selectedCategory ? (
              <div className="flex items-center space-x-3">
                <button onClick={() => setSelectedCategory(null)} className="p-1.5 text-[#5a5852] hover:text-[#f54e00] hover:bg-orange-50 rounded-full transition-all -ml-2">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>
                </button>
                <h2 className="text-xl font-medium text-[#26251e] truncate max-w-[180px]">{selectedCategory.name}</h2>
              </div>
            ) : (
              <h2 className="text-xl font-medium text-[#26251e]">หมวดหมู่ทั้งหมด</h2>
            )}
            <button onClick={() => { setIsSideMenuOpen(false); setTimeout(() => setSelectedCategory(null), 300); }} className="p-2 text-[#5a5852] hover:text-[#26251e] hover:bg-black/5 rounded-full transition-colors">
          ...
          <div className="flex-1 overflow-hidden relative">
                ...
                {categoriesData.map((category, idx) => (
                  <li key={idx}>
                    <button
                      ...
                      className="w-full text-left px-4 py-3.5 text-[#5a5852] hover:text-[#f54e00] hover:bg-orange-50/80 rounded-xl transition-all font-medium flex justify-between items-center group"
                    >
                      {category.name}
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[#a09c92] group-hover:text-[#f54e00] transition-colors"><polyline points="9 18 15 12 9 6"></polyline></svg>
                    </button>
                  </li>
                ))}
              </ul>
            ...
            <div className={`absolute inset-0 p-4 transition-transform duration-300 ease-in-out ${selectedCategory ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'} overflow-y-auto`} style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
              {selectedCategory && (
                <ul className="space-y-1">
                  <li className="mb-3">
                    <a href="#" className="block px-4 py-3.5 text-[#f54e00] bg-orange-50/50 hover:bg-orange-100/50 rounded-xl transition-all font-semibold flex items-center justify-between border border-orange-100">
                  ...
                  {selectedCategory.subcategories.map((sub, idx) => (
                    <li key={idx}>
                      <a href="#" className="block px-4 py-3 text-[#5a5852] hover:text-[#26251e] hover:bg-[#e6e5e0]/50 rounded-xl transition-all font-medium">
                        {sub}
                      </a>
                    </li>
                  ))}
      ...
      <div className={`fixed inset-0 z-[55] flex justify-end transition-all duration-300 ${isCartOpen ? 'visible' : 'invisible'}`}>
        ...
        <div className={`relative w-96 max-w-sm bg-[#f7f7f4]/95 backdrop-blur-xl h-full shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="p-6 border-b border-[#e6e5e0] flex justify-between items-center bg-white/50 backdrop-blur-md">
            <div className="flex items-center space-x-2">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#f54e00" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="9" cy="21" r="1"></circle><circle cx="20" cy="21" r="1"></circle><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"></path></svg>
              <h2 className="text-xl font-semibold text-[#26251e]">ตะกร้าสินค้า (2)</h2>
            </div>
            <button onClick={() => setIsCartOpen(false)} className="p-2 text-[#5a5852] hover:text-[#f54e00] hover:bg-orange-50 rounded-full transition-colors">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
            </button>
          </div>
                ...
                <div key={item.id} className="flex gap-4 group">
                  ...
                  <div className="flex flex-col flex-1 py-1">
                    <div className="flex justify-between items-start">
                      <h3 className="font-semibold text-[#26251e] leading-tight text-sm pr-4 line-clamp-2">{item.title}</h3>
                    ...
                    <div className="mt-auto flex justify-between items-center">
                      <div className="flex items-center border border-[#e6e5e0] rounded-lg bg-white overflow-hidden">
                        <button className="px-2 py-0.5 text-[#5a5852] hover:bg-orange-50 hover:text-[#f54e00] transition-colors">-</button>
                        <span className="px-2 py-0.5 text-sm font-medium text-[#26251e] bg-[#f7f7f4]">1</span>
                        <button className="px-2 py-0.5 text-[#5a5852] hover:bg-orange-50 hover:text-[#f54e00] transition-colors">+</button>
                      </div>
                      <span className="font-bold text-[#f54e00]">${item.price}</span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="p-6 border-t border-[#e6e5e0] bg-white/50 backdrop-blur-md">
            <div className="mb-4 space-y-3">
              <button onClick={() => { setIsCartOpen(false); setTimeout(() => setIsCouponModalOpen(true), 300); }} className="w-full flex items-center justify-between px-4 py-3 bg-orange-50 text-[#f54e00] rounded-xl hover:bg-orange-100/80 transition-colors border border-orange-100">
                <div className="flex items-center space-x-2">
              ...
              <div className="flex space-x-2">
                <input 
                  ...
                  className="flex-1 bg-white border border-[#e6e5e0] rounded-xl py-2.5 px-4 text-[13px] text-[#26251e] focus:outline-none focus:ring-2 focus:ring-orange-300 transition-all placeholder:text-[#a09c92]"
                />
                <button className="px-4 py-2.5 bg-[#26251e] text-white rounded-xl text-[13px] font-medium hover:bg-[#5a5852] transition-colors whitespace-nowrap">
                  ใช้โค้ด
                </button>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center mb-1">
              ...
              <span className="text-[#5a5852] font-medium text-[14px]">${cartTotal.toFixed(2)}</span>
            </div>
            
            <div className="flex justify-between items-center mb-5">
              <span className="text-[#26251e] font-semibold text-[16px]">ยอดสุทธิ</span>
              <span className="text-2xl font-bold text-[#f54e00]">${cartTotal.toFixed(2)}</span>
            </div>
            
            <button className="w-full bg-gradient-to-r from-orange-500 to-amber-500 text-white py-3.5 rounded-2xl font-semibold text-[15px] hover:shadow-lg hover:shadow-orange-500/30 hover:-translate-y-0.5 transition-all duration-300">
              ดำเนินการชำระเงิน
            </button>
            
            <button onClick={() => setIsCartOpen(false)} className="w-full bg-white text-[#5a5852] border border-[#e6e5e0] py-3.5 mt-3 rounded-2xl font-semibold text-[15px] hover:bg-orange-50 hover:border-orange-100 hover:text-[#f54e00] transition-all duration-300">
      ...
      <div className={`fixed inset-0 z-[60] flex items-center justify-center transition-all duration-300 ${isCouponModalOpen ? 'visible' : 'invisible'}`}>
        ...
        <div className={`relative w-full max-w-2xl bg-[#f7f7f4] rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden transition-all duration-300 ease-in-out ${isCouponModalOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-8'} m-4 max-h-[85vh]`}>
          ...
          <div className="p-6 overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-white rounded-2xl border border-[#e6e5e0] flex overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                <div className="bg-orange-100 w-24 flex flex-col items-center justify-center border-r border-dashed border-orange-300 relative">
                  ...
                  <span className="text-orange-600 font-bold text-2xl">20%</span>
                  <span className="text-orange-500 text-[11px] font-semibold">OFF</span>
                </div>
                <div className="p-4 flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="font-semibold text-[#26251e] text-[15px]">ส่วนลดหนังสือใหม่</h3>
                  ...
                  <div className="flex justify-between items-end mt-4">
                    ...
                    <button className="bg-orange-500 hover:bg-orange-600 text-white text-[12px] font-semibold px-4 py-1.5 rounded-full transition-colors">
                      เก็บโค้ด
                    </button>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-2xl border border-[#e6e5e0] flex overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                <div className="bg-blue-50 w-24 flex flex-col items-center justify-center border-r border-dashed border-blue-200 relative">
                  ...
                  <span className="text-blue-500 text-[11px] font-semibold">SHIPPING</span>
                </div>
                <div className="p-4 flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="font-semibold text-[#26251e] text-[15px]">ส่งฟรีไม่มีขั้นต่ำ</h3>
