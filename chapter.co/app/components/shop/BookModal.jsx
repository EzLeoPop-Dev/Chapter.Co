    ...
    <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 transition-all duration-400 ${selectedBook ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
      ...
      {selectedBook && (
        <div className={`relative w-full max-w-4xl bg-white/95 backdrop-blur-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh] transition-all duration-500 ease-out ${selectedBook ? 'translate-y-0 scale-100' : 'translate-y-12 scale-95'}`}>
          ...
          <button onClick={() => setSelectedBook(null)} className="absolute top-5 right-5 z-20 w-10 h-10 bg-black/5 hover:bg-black/10 rounded-full flex items-center justify-center text-[#1A1A1A] transition-colors">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
          </button>

          {/* Left: Book Cover Image */}
          <div className="w-full md:w-2/5 bg-[#F2EEE7] relative flex items-center justify-center p-8 md:p-12 overflow-hidden">
            <div className="absolute top-[-20%] left-[-20%] w-64 h-64 bg-primary rounded-full mix-blend-multiply filter blur-3xl opacity-50"></div>
            <div className="absolute bottom-[-20%] right-[-20%] w-64 h-64 bg-tertiary rounded-full mix-blend-multiply filter blur-3xl opacity-50"></div>
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center space-x-6 mb-8 pb-8 border-b border-[#e6e5e0]">
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center text-[#1A1A1A] font-bold text-lg">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="#C8861A" stroke="#C8861A" className="mr-1.5"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>
                    {selectedBook.rating}
                  </div>
                </div>
                ...
                <div className="flex flex-col">
                  ...
                  <div className="text-[#1A1A1A] font-bold text-lg">{selectedBook.reviews}</div>
                </div>
                <div className="w-px h-10 bg-[#e6e5e0]"></div>
                <div className="flex flex-col">
              ...
              <div className="mb-8">
                <h3 className="text-lg font-bold text-[#1A1A1A] mb-3">เรื่องย่อ (Synopsis)</h3>
                <p className="text-[#1A1A1A] leading-relaxed text-[15px]">{selectedBook.desc}</p>
              </div>
            </div>
